...
export default function EventsList({ events, getEvents }: { events: EventType[], getEvents: (activeFilter: string) => Promise<EventType[]> }) {
    ...
    useEffect(() => {
        Object.keys(filters).map(filterName => {
            if (filters[filterName as keyof typeof filters].length > 0) {
                console.log('filterName, filters[filterName]', filterName, filters[filterName]);
                params.set(filterName, filters[filterName as keyof typeof filters])
